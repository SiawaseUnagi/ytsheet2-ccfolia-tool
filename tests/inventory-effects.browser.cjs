...
(async()=>{let browser;try{
 ...
 assert.equal(await page.locator('#palette').inputValue(),text);assert.equal(await heal().getByRole('checkbox',{name:/：携帯回復具$/,includeHidden:true}).isChecked(),true);assert.equal(await heal().getByRole('checkbox',{name:/：回復装備$/,includeHidden:true}).isChecked(),true);
