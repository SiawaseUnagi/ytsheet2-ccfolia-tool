...
export function mountCalculationEditor(host: HTMLElement, prepared: PreparedPalette, palette: HTMLTextAreaElement, hooks: Hooks, initial = emptyCalculationState()): CalculationEditor {
  ...
  host.append(el("p", "補正を加えたい式を開き、使う効果にチェックを入れてください。新規出力時はすべて未選択です。保存を開いたときは前回の選択を復元します。ゆとシートに反映済みの効果は二重に足さないよう選ばないでください。"));
  ...
  for (const group of groups) {
    ...
    const outer = el("details"); outer.open = true; outer.append(el("summary", `${group.name}（${targets.length}式）`));
    for (const target of targets) {
      ...
      const states = new Map<string, CalculationState["choices"][number]>();
      const preview = el("textarea"); preview.readOnly = true; preview.rows = 3;
      preview.setAttribute("aria-label", `${target.title}の候補式`); preview.style.cssText = "width:100%;box-sizing:border-box;margin-top:8px";
      const message = el("p"); message.setAttribute("aria-live", "polite");
      const selections = (ensure: boolean): Selection[] => candidates.flatMap(modifier => {
        const state = states.get(modifier.id); if (!state?.checked) return [];
      ...
      const rebuild = () => {
        const selected = selections(true); preview.value = renderRoll(target, selected);
        tracker.observe(palette.value);
        if (tracker.replace(target.id, preview.value)) {
          palette.value = tracker.text; message.textContent = "この式の補正を反映しました。最後に「ココフォリアJSONをコピー」を押してください。";
        } else message.textContent = "この式は手で編集されているため、上書きしていません。下の候補式をコピーし、必要な部分だけチャットパレットへ反映してください。";
        summary.textContent = `${target.title}（選択 ${selected.length}件）`;
      ...
      for (const modifier of candidates) {
        const tk = targetKey(target), mk = modifierKey(modifier, prepared.modifiers);
        const saved = initial.choices.find(c => c.target === tk && c.modifier === mk);
        const state = { target: tk, modifier: mk, checked: saved?.checked ?? false, toggle: saved?.toggle ?? modifier.conditional };
        snapshots.push(state); states.set(modifier.id, state);
        const binding = bindings.get(modifier.flag)!;
        ...
        const checkbox = el("input"); checkbox.type = "checkbox"; checkbox.checked = state.checked;
        checkbox.dataset.modifierId = modifier.id; checkbox.setAttribute("aria-label", `${target.title}：${modifier.source}`);
        ...
        const updateFlagHelp = (force = false) => {
          ...
          rename.hidden = !state.toggle; if (force || !rename.open) nameInput.value = name;
        };
        const applyRename = () => {
          try {
        ...
        mode.onchange = () => { state.toggle = mode.value === "toggle"; if (state.checked) rebuild(); else hooks.changed(); updateFlagHelp(); };
        card.append(box);
      }
      preview.value = renderRoll(target, selections(false));
      if (selections(false).length) summary.textContent = `${target.title}（選択 ${selections(false).length}件）`;
