...
export function buildStatus(sheet: ParsedSheet, custom: CustomCommandMap) {
  ...
  const fixed = [
    ["HP", sheet.hp, sheet.hp], ["MP", sheet.mp, sheet.mp], ["フェイト", sheet.fate, sheet.fate],
    ["移動力", sheet.move, 0], ["物理防御力", sheet.phyDef, 0], ["魔法防御力", sheet.magDef, 0],
    ["携帯可能重量", sheet.carry, carryMax], ["判定BD", 0, 0], ["命中BD", 0, 0],
    ["回避BD", 0, 0], ["ダメBD", 0, 0], ["ダメバフ", 0, 0],
  ...
  const raw = sheet.raw;
  for (const item of [...DEFAULT_CONSUMABLES, ...OPTIONAL_CONSUMABLES]) {
    const count = consumableCount(raw, item);
    if (count > 0) addStatus(extras, existing, item.label, count, 0);
  }
  for (const item of detectRiryokufu(raw)) addStatus(extras, existing, item.label, item.count, 0);
  ...
  if (sheet.skills.some(consumesGold)) addStatus(extras, existing, "所持金", safeNumber(raw.moneyTotal ?? raw.money, 0), 0);
  ...
  for (let i = 1; i <= unitStatusNum; i++) {
    ...
    addStatus(extras, existing, label, safeNumber(raw[`unitStatus${i}Value`], 0), 0);
  }
